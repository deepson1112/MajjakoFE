    ...
    useInfiniteQuery({
      queryFn: async ({ pageParam = 1 }) => {
        const response = await api()
          .get(`/retails/retail-product-list/?&page=${pageParam}`)
          .json<ProductShortResponse>();
        return response;
      },
      queryKey: [`other-products-infinity-scroll`],
    ...
    <div className="space-y-4 my-4">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {isLoading ? (
          ...
          <>
            <div className="col-span-full flex items-center justify-between my-1 border-b-[3px] border-gray-200/60 w-full">
              <h3 className="text-gray-600 font-semibold tracking-wider -mb-[0.2rem] pb-3 border-b-[3px] border-brand">
                You may also <span className="text-brand">like</span>
              </h3>
            </div>
            {/* <h3 className="font-semibold text-xl mb-1 col-span-full">
              You may also <span className="text-brand">like</span>
            </h3> */}
            <>
              {data?.pages?.map((page) =>
                page.results?.map((item) => {
                  ...
                  //   parseFloat(item.price) - item.discounted_amount;
                  const discountedPrice =
                    ...
                    >
                      ...
                      <div className="mt-4">
                        ...
                        <div className="flex justify-between">
                          <p className="mt-1 text-brand text-[0.920rem] font-semibold line-clamp-1">
                            <Price
                              amount={
                      ...
                      <div className="mt-2">
                        {!!item.average_rating ? (
                          <ProductRatings rating={item.average_rating} sm />
                        ) : null}
                      </div>
                    </Link>
                  );
