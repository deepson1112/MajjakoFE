  ...
  return (
    ...
    >
      {value.map((item, index) => {
        const option = options.find((x: any) => x.id === parseInt(item));
        const displayText = option ? option.name : item;
...
>(({ className, children }, ref) => {
  return (
    <CommandList
      ref={ref}
      className={cn(
        "p-2 flex flex-col gap-2 rounded-md scrollbar-thin scrollbar-track-transparent transition-colors scrollbar-thumb-muted-foreground dark:scrollbar-thumb-muted scrollbar-thumb-rounded-lg w-full max-w-lg absolute bg-slate-200 shadow-md z-10 border border-muted top-0",
        className
      )}
    >
      {children}
